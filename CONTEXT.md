...
- **`WireMediaFrame`:** L'unico valore che cambia forma attraversando il ponte mobile: il core parla `MediaFrameMessage` con byte grezzi, il telefono parla base64 (la fotocamera lo produce già così e l'unico consumatore è un `Image` con `source` a `data:` URI). La traduzione sta in [media-frame.ts](mobile/src/bare/media-frame.ts), dichiarata nel contratto invece che nascosta in un cast, insieme a `PLAYABLE_FRAME_KINDS` — i tipi di frame che il telefono sa mostrare. L'audio si ferma al confine: mobile non lo cattura né lo riproduce, e i ~31 pacchetti al secondo del desktop venivano codificati, spediti e buttati.
