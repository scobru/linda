...
### `SessionContract`
- **Stato:** Implementato per il percorso worker desktop in [session-contract.ts](src/app/session-contract.ts). Il dispatcher costruisce da lì i suoi inoltri; il proxy (`RemoteSessionView`) resta scritto a mano ma è **enumerato** dai test, che falliscono se un membro manca o se dichiara meno parametri di `Session`. Il worklet mobile ha il proprio contratto in [session-contract.ts](mobile/src/bare/session-contract.ts), perché il trasporto è diverso (tutto asincrono, niente mirror né `Effect`), ma **con lo stesso vocabolario**, e [session-surface-parity.test.ts](test/session-surface-parity.test.ts) li confronta: ciò che una piattaforma offre e l'altra no è una lista dichiarata, con un motivo per voce.
- **Bucket:** Ogni membro di `Session` cade in esattamente uno di questi, e il compilatore lo esige: **inoltrato** (sul desktop con il suo `Effect`), **mirror** (solo desktop: getter sincroni serviti dallo stato che il worker spinge), **adattato** (raggiungibile dalla UI tramite codice scritto a mano, con il motivo), **interno** (lo usa solo il runtime che ospita la sessione, mai la UI), **non esposto** (un buco dichiarato su quella piattaforma).
...
- **Invariante:** Ogni messaggio nuovo di un altro autore viene consegnato agli handler **una volta sola**, in ordine, anche attraverso i riavvii: la posizione di lettura per stanza (`bot-cursors.json`) ricorda l'indice e gli id degli ultimi messaggi gestiti, perché Autobase può riordinare la vista e far ricomparire un messaggio a un indice già superato. La storia precedente al momento in cui il bot ha visto la stanza non viene mai gestita. **Risposte lunghe** ([chunks.ts](src/bot/chunks.ts)): una risposta lunga parte in più messaggi, tagliati tra paragrafi, poi tra righe, poi tra parole, fino a `MAX_MESSAGE_CHARS`; solo il primo è una risposta al messaggio. `ctx.stream()` fa lo stesso man mano che il testo arriva (un paragrafo alla volta, da `MIN_STREAMED_CHARS`) e mostra "sta scrivendo" finché non finisce. **Accesso** ([access.ts](src/bot/access.ts)): con `access.users` il bot ascolta solo quelle identità e rifiuta le richieste di contatto degli altri; con `access.rooms` sta solo in quelle stanze (più le chat dirette con gli utenti ammessi) e rifiuta di entrare altrove prima ancora di farlo. I messaggi esclusi vengono saltati per sempre, come la storia.

### `OpenClawBot`
- **Definizione:** Bridge autonomo verso gateway multi-agente OpenClaw, in [openclaw.ts](src/bot/openclaw.ts).
- **Protocollo:** Connessione HTTP/SSE verso `OPENCLAW_GATEWAY_URL` con autenticazione Bearer.
- **Session Routing:** Chiavi di sessione isolate e canoniche nel formato `agent:<agentId>:linda:<roomId>`.
- **Comandi in-room:** `/agent <agentId>`, `/models`, `/model <modelId>`, `/status`, `/help`. I comandi sono annunciati via bot-profile su desktop e mobile.

### `AIAgent` & `AIAgentStore`
- **Definizione:** Sottosistema per la creazione e interazione con agenti AI sovrani direttamente all'interno delle applicazioni desktop e mobile ([src/ai/](src/ai/)).
- **Moduli:**
  - `AIAgentConfig` ([agent-types.ts](src/ai/agent-types.ts)): Configurazione dell'agente (provider, baseUrl, model, apiKey, systemPrompt, roomId). Preset predefiniti per Ollama (locale), OpenRouter e custom OpenAI-compatible.
  - `AIAgentStore` ([agent-store.ts](src/ai/agent-store.ts)): Store con adapter polimorfo (`localStorage` su desktop, `SecureStore` su mobile).
  - `queryAIAgent` ([agent-runner.ts](src/ai/agent-runner.ts)): Runner HTTP SSE con streaming di token in tempo reale, gestione delta/reasoning content e fallback unificato per runtime mobili.
- **Invariante:** L'agente non necessita di server relay. I messaggi dell'agente vengono firmati e aggiunti alla stanza locale Autobase come autore sovrano con prefisso `agent:<agentId>`. La cronologia risiede su disco nel Corestore crittografato del dispositivo.
