...
import { gun, DAPP_NAME, avatarService } from "#protocol";
...
const Profile = ({ isOpen, onClose }) => {
  ...
  const loadCustomAvatar = async () => {
    try {
      ...
      const avatarData = await avatarService.getAvatar(currentPub);
      if (avatarData) {
  ...
  const saveAvatar = async () => {
    try {
      ...
      await avatarService.saveAvatar(currentPub, customAvatar);
