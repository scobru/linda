...
import { AVATAR_PRESETS, matchesPreset } from './avatar-presets.js'
...
export class AppShell extends HTMLElement {
  ...
  private async enterApp(isNewIdentity = false): Promise<void> {
    ...
    try {
      const session = await openSession(this.identity, storageDir(), { events: {
        ...
        onIncomingCall: (info) => this.callOverlay.handleIncomingCall(info),
  ...
  private wireRoom(): void {
    ...
    this.querySelector('#startVoiceCallBtn')?.addEventListener('click', () => {
      if (callPeerId) void this.callOverlay.startCall(callPeerId, room.id, { audio: true, video: false })
    })
    this.querySelector('#startVideoCallBtn')?.addEventListener('click', () => {
      if (callPeerId) void this.callOverlay.startCall(callPeerId, room.id, { audio: true, video: true })
    })
  ...
  private renderProfilePage(): void {
            ...
            <div class="form-group">
              ...
              <div class="preset-avatars-grid">
                ${AVATAR_PRESETS.map((p) => `
                  <button class="preset-avatar-card ${matchesPreset(p, workingAvatar) ? 'active' : ''}" data-preset-id="${p.id}" title="${p.name}">
    ...
    this.querySelectorAll<HTMLButtonElement>('.preset-avatar-card').forEach((btn) => {
      btn.addEventListener('click', () => {
        const preset = AVATAR_PRESETS.find((p) => p.id === btn.dataset.presetId)
        if (preset) {
