...
export default function Profile() {
  ...
  React.useEffect(() => {
    const getUserInfo = async () => {
      try {
        ...
        gun.user().once((userData) => {
          if (userData) {
            setPub(userData.pub || user.is.pub);
            setAlias(userData.alias || user.is.alias);
          }
        });
      } catch (error) {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      // Esegui il logout
      const logoutSuccess = await authentication.logout();
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <h2 className="text-sm font-medium truncate">{alias}</h2>
